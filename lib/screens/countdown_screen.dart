...
class _CountDownState extends State<CountDown> {
  ...
  timerUpdate() {
    if (timeLeft <= 0) {
      //sendTestingReport('time less than 0');
  ...
  void startAlertTimer() async {
    ...
    Duration timeOut = Duration(
        minutes: askTimerSnooze); //Duration(minutes:askDuration.inMinutes);
  ...
  clockTick() {
    ...
    countDown = Timer.periodic(Duration(seconds: 1), (Timer t) {
      ...
      //sendTestingReport('counting '+timeLeft.toString());
  ...
  void startNotificationTimer() async {
    ...
    //sendTestingReport('start notif timer alarm here');

    // await askPlayer.setAsset('assets/audio/alarm.mp3');
    // askPlayer.setLoopMode(LoopMode.one);
    print('just before play');

    startAlertTimer();

    // if (stateNotification.index == 2){
    //   //showNotification(resetTimer, 1);
    //   //sendTestingReport('start alert state 2');
    //   startAlertTimer();
    // }else{
    //   //timerDialog();
    //   //sendTestingReport('start alert normal');
    //   startAlertTimer();
    // }
    showAskPopup = true;
