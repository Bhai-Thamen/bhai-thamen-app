...
camPictures = FirebaseStorage.instance.ref().child('user_images').child(firebaseuser.uid).child(madeDate) ;


    StorageUploadTask  storageUploadTask = camPictures.child(id).putFile(theImageFile);
...
checkLocalFiles()async{
  ...
  print ('FILES HERE ________________');
  mainOne.list(recursive: true, followLinks: false)
...
var expiryDate = DateTime.utc(2020, 12, 31);
...
prefs.setBool('expired', expired);
    ...
    Route route = MaterialPageRoute(builder: (context) => MapWrapper(),settings: RouteSettings(name: 'Mapping'),);
    ...
    if (userData==null){ return CircularProgressIndicator();}
      ...
      Center(child:
              ...
              Column(
                children: [
              SizedBox(height:15),                  
              Text(languages[selectedLanguage[languageIndex]]['welcome'], style: myStyle(35), textAlign: TextAlign.center,),
                ...
                ButtonTheme(
                  ...
                  Navigator.push(
                   ...
                   // padding: EdgeInsets.only(left: MediaQuery.of(context).size.width/4, right: MediaQuery.of(context).size.width/4, top: 15, bottom:15),
                    ...
                    child: Text(languages[selectedLanguage[languageIndex]]['setupProfile'], style: myStyle(26, Colors.white),),
                    ),
                ),
                SizedBox(height:10),
              ],
            ...
            if (userData.phoneContact.length>0) 
                for (var i=0; i<userData.phoneContact.length; i++){
                   bool done = await sendNewSms (userData.phoneContact[i], userData.userName, i, 'button');
                  print ('done $i'+done.toString());
                  }
                  smsBtnSending();
                  sendEvent('sms','user-sms');  
                  ...
                  child: Container(
                    ...
                    child: 
                    languageIndex == 0 ? Image.asset(
                 'assets/images/sosSMS.png', 
                width: 80,
                height: 80,
                fit: BoxFit.fill,
              ):
                 Image.asset('assets/images/sosSMSBN.png',
                width: 80,
                height: 80,
                fit: BoxFit.fill,
                 ), 
            ...
            ButtonTheme(
              ...
              child: RaisedButton(
               ...
               // padding: EdgeInsets.only(left: MediaQuery.of(context).size.width/4, right: MediaQuery.of(context).size.width/4, top: 15, bottom:15),
                ...
                child: Text(languages[selectedLanguage[languageIndex]]['safetyZones'], style: myStyle(26, Colors.white),),
                ),
            ),                  
                ],
              ),
